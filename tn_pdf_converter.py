...
from pdf_converter import RepresentsInt, DEFAULT_ULT_ID, DEFAULT_UST_ID
...
class TnPdfConverter(TsvPdfConverter):
    ...
    def populate_tn_book_data(self):
        ...
        for row in reader:
            ...
            occurrence = 1
            if RepresentsInt(verse_data['Occurrence']) and int(verse_data['Occurrence']) > 0:
...
'''
    ...
    def get_scripture_with_tn_quotes(self, bible_id, chapter, verse, rc, scripture):
        ...
        for tn_note_idx, tn_note in enumerate(tn_notes):
            occurrence = 1
            if RepresentsInt(tn_note['Occurrence']) and int(tn_note['Occurrence']) > 0:
