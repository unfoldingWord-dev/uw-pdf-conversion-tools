 ...
 */
export function getMissingResources(sourcePath, resourcesPath, getMissingOriginalResource, downloadErrors, languageId) {
  ...
  for (const isNewTestament of [false, true]) {
    ...
    if (query) {
      ...
      getMissingOriginalResource(resourcesPath, origLangId, origLangBibleId, origLangVersion, downloadErrors);
 ...
 * @param {Array} downloadErrors - parsed list of download errors with details such as if the download completed (vs. parsing error), error, and url
 */
export function processTranslationNotes(resource, sourcePath, outputPath, resourcesPath, downloadErrors) {
  try {
    ...
    const {otQuery, ntQuery} = getMissingResources(sourcePath, resourcesPath, getMissingOriginalResource, downloadErrors, resource.languageId);
    ...
    tsvFiles.forEach((filename) => {
      try {
        ...
        if (resource.book && resource.book != bookId) {
          return;
        }
        ...
        if (fs.existsSync(originalBiblePath)) {
          ...
          const jsonData = tsvToGroupData(filepath, 'translationNotes', { categorized: true }, originalBiblePath, resourcesPath, resource.languageId);
          formatAndSaveGroupData(jsonData, outputPath, bookId);
        } else {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (!fs.existsSync(originalBiblePath)) {
        ...
        delay(1000);
        downloadAndProcessResource(resource, resourcesPath, downloadErrors);
        resolve();
      } else {
        resolve();
      }
