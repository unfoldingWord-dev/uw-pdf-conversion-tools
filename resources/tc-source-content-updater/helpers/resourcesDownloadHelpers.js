 ...
 */
export const downloadAndProcessResource = (resource, resourcesPath, downloadErrors) => {
  if (!resource) {
  ...
  try {
    try {
      ...
      const results = downloadHelpers.download(resource.downloadUrl, zipFilePath);
    ...
    try {
      ...
      importPath = unzipResource(resource, zipFilePath, resourcesPath);
    } catch (err) {
    ...
    const processedFilesPath = processResource(resource, importSubdirPath, resourcesPath, downloadErrors);
    if (processedFilesPath) {
      ...
      try {
        moveResourcesHelpers.moveResources(processedFilesPath, resourcePath);
      } catch (err) {
 ...
 */
export const downloadAndProcessResourceWithCatch = (resource, resourcesPath, errorList, downloadErrors) => {
  let result = null;
  try {
    result = downloadAndProcessResource(resource, resourcesPath, downloadErrors);
